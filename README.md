...
## Content 
Assume T = \sum_{j\in\mathbb{N}} T_j where T,T_j are linear operators that map measurable functions defined on a measure space X to measurable functions defined on a measure space Y. Based on the input data the program generates, amongst others, a pdf file describing and depicting the region of exponents p,q\in [1,\infty] where estimates of the form \|Tf\|_{L^q(Y)}\leq C\|f\|_{L^p(X)} or variants thereof hold. This is based on real and complex interpolation theory and the assumptions on X,Y are such that the standard theorems (see the book of Bergh-Löfström) applies. 
Riesz diagrams are used to visualize the result. 

## General information:
The input data is processed using rational numbers only in decimal of fractional form. The given estimates may depend on some non-specified parameter d in a "simple" way, i.e., using +,-,* and /. The computations and plots, however, are executed only for specific values of integer values of d that the user may specify as well. The default values of some input fields (see below) permit to produce an output just by inserting the estimates.


## How to enter your data through the html:
(In the following, the quotation marks "" indicate the begin/end of the input string. Please do not type them.)

* Views.Content:  A finite sequence of elements of \{1,2,3,4,5\}. It stands for the kind of estimates the user is interested in.
...
Interpolation requires at least two valid estimates, so the input data must lead to such. Otherwise the program will run into an error.
